...
import {Box, Checkbox, PageTitle, IngredientBox, SmallMealBox} from '../UI/Layouts';
import {
    Wrapper,
    IngredientGrid,
    EditIcon,
...
export default function Pickup(props) {
    ...
    <SmallMealBox meal={meal} title={<TrashButton onClick={deleteFilter}><TrashIcon/></TrashButton>} actions={ <EditButton onClick={handleIngredients}><EditIcon/></EditButton>}>
        ...
        <>
            ...
            <Block><MealName>{`${meal.name} (${meal.price[0]}): $${meal.price[1]}.00 `}</MealName></Block>
        </>
        }
        {toggleIngredient && 
        <Wrapper>
            <IngredientBox title={`${meal.name} ingredients: `}>  
            <Table>
                <thead>
                    <tr>
                        <th><input onChange={e => {
